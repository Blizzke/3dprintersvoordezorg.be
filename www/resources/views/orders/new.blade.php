...
<form method="post" action="/order/new" class="form-horizontal">
    ...
    <div class="form-group">
        ...
        <div class="col-sm-3">
            <input type="text" name="customer_id" id="customer_id" class="form-control" value="{{ old('customer_id') }}"/>
        </div>
        <div class="col-sm-6">
            (indien je het hebt: klantnummer van een vorige bestelling, je mag de rest van de gegevens/contact leeg laten)
        </div>
    </div>
    <div class="form-group">
        ...
        <div class="col-sm-9">
            ...
            <label class="radio-inline">
                <input type="radio" name="sector" id="{{$id}}" value="{{$id}}" {{ old('sector')==$id ? 'checked' : '' }}> {{$name}}
            </label>
            @endforeach
            @error('sector')<div class="alert alert-danger">{{ $message }}</div> @enderror
        </div>
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
        ...
        @foreach($items as $item)
            <div class="radio">
                <label>
                <input type="radio" name="item" value="{{$item->name}}" {{ old('item')==$item->name ? 'checked' : '' }}>
                    <b>{{$item->title}}</b><br/>
        ...
        @endforeach
        @error('item')<div class="alert alert-danger">{{ $message }}</div> @enderror
        </div>
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
        ...
        <div class="col-sm-9">
            <textarea type="text" name="comment" id="comment" rows="5" class="form-control">{{ old('comments') }}</textarea>
        </div>
    </div>
    <div class="form-group">
        <div class="col-sm-12">
            <button class="btn btn-success btn-submit">Bestel</button>
        </div>
    </div>
</form>
